...
@invocation(
    "nightmare_promptgen",
    title="Nightmare Promptgen",
    tags=["nightmare", "prompt"],
    category="prompt",
    version="1.4.1",
    use_cache=False,
)
class NightmareInvocation(BaseInvocation):
    ...
    prompt: str = InputField(
        default="", description="starting point for the generated prompt", ui_component=UIComponent.Textarea
    )
    split_prompt: bool = InputField(default=False, description="If the prompt is too long, will split it with .and()")
    max_new_tokens: int = InputField(
        default=300, ge=3, le=1200, description="the maximum allowed amount of new tokens to generate"
    )
    min_new_tokens: int = InputField(
        default=30, ge=0, le=800, description="the minimum new tokens - NOTE, this can increase generation time"
    )
    max_time: float = InputField(
        default=10.0, ge=5.0, le=120.0, description="Overrules min tokens; the max amount of time allowed to generate"
    )
    temp: float = InputField(default=1.8, ge=0.5, le=4.0, description="Temperature")
    typical_p: float = InputField(
        default=1.0, ge=0.1, le=4.0, description="Lower than 1.0 seems crazier, higher is more consistent."
    )
    ...
    repetition_penalty: float = InputField(
        default=1.0, ge=0.5, le=3.0, description="Higher than 1.0 will try to prevent repetition."
    )
    repetition_penalty: float = InputField(
        default=1.0, ge=0.5, le=3.0, description="Higher than 1.0 will try to prevent repetition."
    )
    instruct_mode: bool = InputField(
        default=False, description="If you are using an 'instruct/chat' style model, enable this."
    )
    ...
    def censor(self, phrase: str) -> str:
    ...
    def splitPrompt(self, text: str) -> list[str]:
    ...
    def makePrompts(
        ...
        generator = self.loadGenerator(str(self.repo_id), task)
    ...
    def invoke(self, context: InvocationContext) -> NightmareOutput:
        ...
        if len(generated) > 200 and self.split_prompt:
            ...
            generated = "".join(
                ch for ch in generated if category(ch)[0] != "C"
            )  # further clean up weird control characters
